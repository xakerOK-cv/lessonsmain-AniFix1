    ...
    def __str__(self):
        if self.last_action == 'eat':
            ...
            match self.last_action:
                case 'work':
                    result = f"{self.name} попрацював."
                case 'gaming':
                    result = f"{self.name} пограв."
                case 'buy_food':
                    result = f"{self.name} купила їжі."
                case 'buy_fur_coat':
                    result = f"{self.name} купила шубу."
                case 'not_buy_fur_coat':
                    result = f"{self.name} не вистачило на шубу."
                case 'clean_house':
                    result = f"{self.name} віддраяла будинок."
                case _:
...
#           print(f'При зарплате {salary} максимально можно прокормить {max_cats} котов')# Еда стоит 10 денег 10 единиц еды. Шуба стоит 350 единиц.
#
# Грязь добавляется каждый день по 5 пунктов, за одну уборку жена может убирать до 100 единиц грязи.
# Если в доме грязи больше 90 - у людей падает степень счастья каждый день на 10 пунктов,
# Степень счастья растет: у мужа от игры в WoT (на 20), у жены от покупки шубы (на 60, но шуба дорогая)
